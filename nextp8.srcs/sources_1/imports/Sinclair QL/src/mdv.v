...
always @(negedge mem_ena)  mdv_din <= mem_din;

reg mdv_next_word,mdv_next; 

always @(posedge mdv_next_word or posedge mem_ena) begin
    if (mem_ena) begin mem_read<=mdv_next; end
    else mem_read <= 1'b1;
end
...
reg mdv_clk=1'b0;
reg [4:0] mdv_clk_cnt=5'd0;
always @(posedge mem_clk) begin
    if(mdv_clk_cnt == 5'd6 ) begin  
        mdv_clk_cnt <= 5'd0;
        mdv_clk <= !mdv_clk;
    end else
        mdv_clk_cnt <= mdv_clk_cnt + 5'd1;
end
