	...
	*/
	...
	if ((m_bitpos&7)==0) {
		...
		if (m_reading_data) {
			...
			// Check the CRC
			if (m_crc_on && !check_cmdcrc(m_cmdbuf)) {
				assert(0 && "BAD CRC");
				m_rspbuf[0] = 0x09;
				m_rspdly = 1;
...
// fprintf(stderr, "WRITE: Seek to sector %d\n", arg);
				...
				case 16: // CMD16 -- SET_BLOCKLEN
					if (arg == 512)
						...
						m_rspbuf[0] = 1 << 6; // R1, illegal command
					m_rspdly = 2;
					break;
			...
			// {{{
			assert((m_dat_in&0x0ff) == 0x0ff);
			//printf("m_blkidx: %d -> %d\n", (int) m_blkidx, (int) (m_blkidx + 1));
			m_dat_out = m_block_buf[m_blkidx++];
			if (m_blkidx == m_blklen) {
				printf("m_bklidx = %d BLOCK DONE - %d\n", (int) m_blkidx, (int)(m_cmdbuf[0]&0x3f));
				if ((m_cmdbuf[0]&0x3f) == 18) {
					printf("CMD18 - READ_MULTIPLE_BLOCK - send next block\n");
					memset(m_block_buf, 0x0ff, SDSPI_MAXBLKLEN);
					m_block_buf[0] = 0x0fe;
					m_blklen = SECTOR_SIZE; //(1<<m_csd[5]);
					if (m_dev)
